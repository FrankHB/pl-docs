...
* **显著增加**内存占用：
	* 在内存紧缺的环境下难以使用。
	* 可能导致更高的能耗。
	* 资源利用率低下：典型地，要实现相同的流畅响应，使用 GC 可能相对不使用 GC 需要占用 5 倍的内存，考虑能耗问题，使硬件的选择更加困难。[在移动 Web 应用上这些问题尤其突出。](http://sealedabstract.com/rants/why-mobile-web-apps-are-slow/)
...
可见，有意保留或放任缺少不完整的解显然使用只会更困难，是一种瑕疵(artifact) ；同时，因为依赖这种性质对程序实现而言无法避免一般意义上的可观察行为（包括通常不由语言限制的，典型地，像内存占用这类性能问题）的改变，有向最终用户增加抽象泄漏和降低可用性的风险。因此，解决这种问题往往有较大的实际意义。
...
* 闭包指词法闭包(lexical closure) 。要求完全排除所有者的闭包的概念定义并不直接被“闭包”概念的内涵覆盖。
* 捕获没有所有权的变量访问是可被语义允许的，违反生存期规则时可直接指定为未定义行为。良定义(well-defined) 的访问方式仍然不限制正常的使用。事实上， ISO C++ 的闭包类型（ lambda 表达式的类型）即为这种情形。
* 除上述 ISO C++ 的反例，要求闭包只能具有良定义(well-defined) 的行为这点在外延上也欠缺历史依据。
...
　　从[行为多态(behavioral polymorism)](https://zh.wikipedia.org/zh-cn/子类型) 的角度讲，一个释放资源操作时机不确定的对象（蕴含被 GC 管理的对象）的类型，不应是一个具有确定性释放操作的子类型。因此，[某些对 C# 的 Dispose 的滥用可能性的评论](https://www.zhihu.com/question/51592470/answer/126681206)并非毫无根据。
...
* 关于[原始的 λ 演算中要求闭包的概念的问题](http://lambda-the-ultimate.org/node/5007#comment-81724)，补充以下说明。
	* 考虑图规约(graph reduction) 是多余的。至少在这个上下文中，图规约只是可选的次要优化实现策略，不改变可实现的语义。
	* 另一个关键是创建闭包时的捕获(capture) （确定需要自由变量的集合）没有在 λ 演算这个对象语言中引起计算作用，可以任意地退化（捕获并非当前上下文中并不需要保存的变量）而不影响语义。
	...
	* 在允许捕获对象引起可观察行为的语言中，捕获操作可能有显式的计算作用，这种作用可以有实际意义。例如，C++ 的 `lambda-expression` 的 `capture` [可显式初始化捕获的变量](http://www.eel.is/c++draft/expr.prim.lambda#capture-6)，捕获和捕获之前的初值符求值都可能具有副作用。这种存在明确同一性的可捕获对象的情形中，即便不讨论捕获操作，单独的捕获也不是冗余的：销毁对象可能有计算作用。若要求闭包中存储的自由变量被引用的对象的极小集合完全决定，则不可能有这样的扩展设计，而**限制语言使用一等对象的抽象能力**。根本地，**通过引用来指定（推断）自由变量集合的做法，和表达对象同一性的设计冲突**。即便不考虑计算上的开销和可能不清晰的非预期的表达的问题，这种设计也是可疑的。
