...
## 补充注记 2018-09-15 06:06

补充坑着的关于闭包(closure) 概念的定义问题以及隐藏在背后的另一个价值判断上的哲学问题：关于语言设计的普遍性和其它设计的权衡问题。

为减少铺垫，使用简化为互斥递归(mutual recursive) 的 `letrec` 递归构造的外部所有者是否必要的问题来入手展开讨论。具体例子参见 [LtU 的讨论](http://lambda-the-ultimate.org/node/5007#comment-81721) 。

这里，外部所有者（即 `let` 项）是否必要？若遵循其中 Thomas Lord 的意见，闭包必须（通过图跟踪求解）解决互递归引用求解未知所有者的问题，所有者是不必要的。这样的设计蕴含（跟踪） GC 的必要性，除非允许涉及互递归引用的资源全部泄漏。反之，使用外部明确的所有者避免了遍历跟踪操作，尽管类比 [[Clinger98]](https://www.researchgate.net/profile/William_Clinger/publication/2728133_Proper_Tail_Recursion_and_Space_Efficiency/links/02e7e53624927461c8000000/Proper-Tail-Recursion-and-Space-Efficiency.pdf) 的定义严格意义上仍然可在内部泄漏泄露。

之后的讨论中， Thomas Lord 指出动态确定的项无法期望静态确定所有者。不过，这并没有明确设计中需要考虑的问题：这种情况是否有必要通过不要求所有者的语言特性的设计来支持？

这里尝试分析具体的问题并权衡特性设计以给出确切的“是否要求 GC 是足够合理的”这一有关语言设计决策的结论。答案是否定的，或者至少相当显著地依赖语言设计者在选择特性时的主观动机。

从一般用例看，完全动态生成 `letrec` 构造的项并不常见；相反，静态确定的项相比较下相当常见（这也是支持静态确定所有者的理由之一）。这种状况如果不是随机的，很可能跟用户避免动态互递归的代码构造相关（也不容易找到其它理由）。

避免互斥递归代码的理由可能有很多，例如用户不熟悉语用，不容易找到用例，等等。但是有一定应该容易是共识：除非问题的描述中带有互斥递归，一个互斥递归形式代码的解，往往对应着一个去除互斥递归的更直观解的形式。

这种去除互斥递归的过程，相当于用户执行某种图跟踪算法对具有互斥递归构造的代码进行变换，消除其中的互斥递归构造。更进一步地，这实际上就是静态地明确所有者的过程，和 GC 的主要差别是由代码作者而非运行时进行，并且针对特定问题考虑而不那么复杂也不那么通用。

这里的关键是，只要允许（即互斥递归不是问题的运行时输入），静态化所有者这实际上是**求解问题**的一部分。缺乏这个步骤，一定程度上，问题没有完全地解决，而留待运行时由语言实现解决。这种完全解决的形式，此处称为静态所有者范式(static owner norm) 。

如果问题的解（程序）仅仅是被机器运行的，那么从工程上考虑，这无可厚非：人的时间比机器时间可能更宝贵，用机器时间换取人的时间可能是合理的（如果用户对他们的时间被用来交换程序员的时间这个策略无法表示意见的话）。

但是很多时候，程序作为解，不仅仅是给机器的输入，也需要被人阅读。考虑到通用的运行时 GC 的工作并不容易被单独分离而被读者直接“调用”，读者通常需要特设地(ad-hoc) 自行充当 GC 的角色。考虑人模拟 GC （即便仅是特定问题上）的低效，这在不限制这种没有完全解答的解法的情形下，很可能每次阅读程序都会消耗不少时间和精力。而这本可以通过强调要求一开始的代码作者明确所有者得到静态所有者范式来解决。
...
另一个次要的方面， Thomas Lord 认为要求支持闭包即要支持这种普遍的过程不能被的编写代码的用户静态地执行。这种理解是有问题的，因为：

* 闭包指词法闭包(lexical closure) 。要求完全排除所有者的闭包的概念定义并不直接被内涵覆盖。
* 捕获没有所有权的变量访问是可被语义允许的，违反生存期规则时可直接指定为未定义行为。良定义(well-defined) 的访问方式仍然不限制正常的使用。事实上， ISO C++ 的闭包类型（ lambda 表达式的类型）即为这种情形。
* 要求闭包只能具有良定义的行为在外延上也欠缺历史依据。详见 [closure(en-US)](https://en.wikipedia.org/wiki/Closure_%28computer_programming%29#History_and_etymology) 。
