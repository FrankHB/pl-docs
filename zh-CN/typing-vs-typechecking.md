...
　　现实的类型安全一般在语言设计中由两类手段提供支持：语言的构造性规则限制不安全类型构造的表达，以及语言对潜在不安全的表达进行额外的语义检查。前者涉及对不特定的项（如表达式）进行类型指派，也就是一般所谓的 typing ；后者相对比较事后诸葛亮但在设计语言时配置起来比较灵活（可以很容易地弱到没有而不那么容易影响其它语义），所谓 typechecking ；然而很多人根本没搞清楚**这两者根本不是一回事**。
