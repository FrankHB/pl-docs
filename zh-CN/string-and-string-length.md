...
　　以下引用中文喂鸡“[字符串](https://zh.wikipedia.org/wiki/字符串)”：
...
　　以下引用中文喂鸡“[字符串->字符串数据类型](https://zh.wikipedia.org/wiki/字符串#.E5.AD.97.E7.AC.A6.E4.B8.B2.E6.95.B0.E6.8D.AE.E7.B1.BB.E5.9E.8B)”：
...
　　一种常用的表示法是使用一个字符代码的数组，每个字符通常占用一个字节（如在 ASCII 代码中）或两个字节（如在 UCS-2 这样的 Unicode 表示中）。它的长度可以使用一个结束符（一般是 [NUL](https://zh.wikipedia.org/wiki/空字符) ， ASCII 代码是 0 ，在C编程语言中使用这种方法）。或者在前面加入一个整数值来表示它的长度（在 Pascal 语言中使用这种方法）。
