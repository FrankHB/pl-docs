...
The standard library has some cases using `unsigned long long` to represent unsigned interger with a largest range. This should be not a good practice following the reasoning (and it should be `std::uintmax_t` instead).
...
It may be good to know the roles they have played historically (and some contemporary implementations are worth a look), but relying on them is rather dangerous without any further guarantees. Once you have relied on such guarantees, the resulted programs would be less portable. Usually, keep away from them unless really necessary.
