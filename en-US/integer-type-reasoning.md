...
That depends on what is "use". In C++ terms, you can avoid [*odr-used*](https://eel.is/c++draft/basic.def.odr#4) values without explicitly written forms, e.g. by some instantiations of `std::common_type`. Burying them in generic code is quite easy, if not error-prone.
...
BTW, also note the initial adoption of the notion "register" in the (C) language is only significant historically for implementations. Today the keyword `register` is even ignored, and [it even may have some other meanings some day](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2018/p0577r1.pdf) (*disclaimer: no endorsement or championship is implied here*), as every C++ programmers had better know.
